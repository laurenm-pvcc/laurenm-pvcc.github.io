...
def read_data_file():
    data_file = open(student_file, "r") #The "r" indicates this is a READ file
    lines = data_file.readlines() #Read the entire file into a variable called lines
    for i in lines:
...
def calculate_grades():
    ...
    for i in range(len(stu)):
        ...
        midex = int(stu[i][3]) #Midterm exam is position 3
...
def create_report_file():
    ...
    for i in range (len(stu)):
        ...
        midex = str(stu[i][3])
        finex = str(stu[i][4])
        fingrade = format(stu[i][5], '7,.2f')
        repfile.write("\n" + sname + "\t" + midex + "\t" + finex +"\t" + fingrade + " " + stu[i][6])
    repfile.write(line)
    repfile.close()
    print("Open this file to view the Grades Reprort: " + out_file)
